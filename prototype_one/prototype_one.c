...
int main()
{
    ...
    GPIOA_ODR |= (1<<5); // temporary test. If this turns on then the stuff beforehand worked (in theory)
    while(1);
}
...
{
    RCC_AHB1ENR |= GPIOCEN;
    GPIOC_MODER &= ~(0b11 << BUTTON_PIN_MODER);
    GPIOC_PUPDR &= ~(0b11 << BUTTON_PIN_PUPDR); // Clearing before setting input type
...
void configure_button_interrupt(void) /* Set it so pushing the button results in an interrupt */
{
    ...
    SYSCFG_EXTICR4 |= (0b0010U << BUTTON_PIN_CR4); // Set it for port C pin 13
    ...
    NVIC_ISER1 = (1U << EXTI13_IQR_POS); // interrupt enable
